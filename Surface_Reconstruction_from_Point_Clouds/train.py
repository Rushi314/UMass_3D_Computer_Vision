...
def train(dataset, model, optimizer, args):
    ...
    for i in range(num_batch):
        ...
        out1 = model(xyz_tensor)
        c = args.clamping_distance
        loss = torch.sum(torch.abs(torch.clamp(gt_sdf_tensor, -c, c) -torch.clamp(out1, -c ,c)))
        loss_sum += loss
        (loss/len(out1)).backward() #Backpropagating the average loss
        optimizer.step()
...
def val(dataset, model, optimizer, args):
    ...
    for i in range(num_batch):
        ...
        with torch.no_grad():
            xyz_tensor = data['xyz'].to(device)
            gt_sdf_tensor = data['gt_sdf'].to(device)
            out = model(xyz_tensor)
            c = args.clamping_distance
            loss_sum += torch.sum(torch.abs(torch.clamp(gt_sdf_tensor, -c, c) - torch.clamp(out, -c ,c)))
            loss_count += xyz_tensor.shape[0]
        # ***********************************************************************
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        train_loss = train(train_dataset, model, optimizer, args).float()
        val_loss = val(val_dataset, model, optimizer, args).float()
        scheduler.step()
